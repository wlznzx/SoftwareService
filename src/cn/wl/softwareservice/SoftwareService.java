...
public class SoftwareService extends Service {
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		if(_boot){
			...
			// 2.1 如果在倒车,启动后拉界面.
			if(_isCvbsIn){
				if(DEBUG)Log.d(TAG," onStartCommand 2.1");
				doStartCVBSActivity();
				mHandler.sendEmptyMessageDelayed(MSG_DET_CVBS_WHEN_FRIST, 500);
			}
			// 2.2 没有倒车,开机启动DVR.
			else{
				if(DEBUG)Log.d(TAG," onStartCommand 2.2");
				doBootDVR();
	...
	private void doDetCvbs(){
		...
		if(isCvbsIn != _isCvbsIn){
			isCvbsIn = _isCvbsIn;
			if(isCvbsIn){
				boolean isRunning = isServiceRunning(SoftwareService.this,DVR_SERVICE_NAME);
				// 如果Service存在,发送广播由DVRService处理.
				if(isRunning){
					if(DEBUG)Log.d(TAG," with Service."); 
					sendBroadcast(new Intent(ACTION_INTENT_CVBS_IN));
				}
				// 如果Service不存在,启动CVBSActivity.
				else{
					if(DEBUG)Log.d(TAG," without Service.");
					doStartCVBSActivity();
				}
			}else{
				sendBroadcast(new Intent(ACTION_CVBS_OUT));
	...
	private void doBootDVR(){
		...
		dvrIntent.setComponent(new ComponentName(DVR_PACKAGE_NAME, DVR_MAIN_CLASSNAME));
